...
async function analyzeEmbeddingFailures() {
    ...
    try {
        ...
        // Note: Azure Search ne permet pas de filtrer directement sur les vecteurs nuls
        // Utilisons une approche différente pour identifier les problèmes
        
        // Rechercher tous les documents et examiner ceux qui pourraient avoir des problèmes
        console.log('   Recherche de tous les documents pour analyse...');
        const allDocsResults = await searchClient.search('*', {
        ...
        for await (const result of allDocsResults.results) {
            totalDocs++;
            // Les documents sans embedding auraient des descriptions très courtes ou vides
            const desc = result.document.description || '';
            if (desc.length < 20) {
                console.log(`   ⚠️  ${result.document.legalIdentifier}: desc très courte (${desc.length} chars)`);
                missingVectorCount++;
            }
        }
        console.log(`   Total analysé: ${totalDocs} documents, ${missingVectorCount} suspects`);;
        ...
        console.log('=====================================');
        console.log(`❌ ${missingVectorCount} documents avec embeddings suspects`);
        console.log(`⚠️  ${shortDescCount} documents avec descriptions courtes`);
        console.log(`📊 ${totalDocsForPatterns} documents total analysés`);
