...
export class AzureAISearchDataSource {
    ...
    async renderContext(query) {
        ...
        const selectedFields = [
            ...
            "title",
            "content",
            "url",      // URL blob (not used for display - internal reference only)
            "filepath", // Chemin du fichier source (for reference)
        ];
        ...
        for (const result of filteredResults) {
            ...
            // IMPORTANT: Do NOT use the 'url' field from index (blob storage URL)
            // Instead, pass title/filepath/content and let agent.js:
            // 1. Extract readable title from content using LLM
            // 2. Transform to public URL (legisquebec.gouv.qc.ca or canlii.org)
            
            citations.push({
                title: result.document.title || `Document ${resultCount}`,
                filepath: result.document.filepath || null, // For reference only
                content: result.document.content || '', // For title extraction
                url: null, // Will be generated by transformToLegisQuebecUrl in agent.js
                score: result.score
            });
            
            debugLog('SEARCH', `[CITATION] ${resultCount}: "${result.document.title}" - Title and URL will be processed`);
        }
